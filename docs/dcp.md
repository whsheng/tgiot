...
+ topic中消息内容必须是`JSON`格式，消息内容是由：消息版本号(即通讯规约版本号)+消息头+消息体，这三部分构成；举例如下：
...

{
  ...
  "msg_body":{
    "params":[
    {"参数ID":"0x0001","参数值":"真实取值"},
    {"参数ID":"0x0002","参数值":"真实取值"},
    {"参数ID":"0x0003","参数值":"真实取值"},
...
5. 举例：{"cmd":"AP","timestamp":141411,"ip":"112.23.212.12","port":3221}
